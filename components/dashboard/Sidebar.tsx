...
export default function Sidebar({ isAdmin }: { isAdmin: boolean }) {
  ...
  const ACCOUNT_ITEMS = isAdmin ? ADMIN_ACCOUNT_ITEMS : STUDENT_ACCOUNT_ITEMS;
      ...
      {items.map((item) => {
        ...
        return (
          <li key={item.href} className="relative" suppressHydrationWarning>
            {isActive && (
              <div className="absolute top-1/2 -translate-y-1/2 -right-4 w-[3px] h-8 bg-[#0070F0] rounded-l-full" />
            )}
            <Link
              href={item.href}
            ...
            >
              <item.icon
                className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-[#0070F0]" : "text-slate-500"}`}
              />
              {item.label}
            </Link>
          </li>
    ...
    <aside className="hidden md:flex w-64 flex-col bg-white border-r border-slate-100 fixed h-full z-40">
      ...
      <nav className="flex-1 px-4 py-2 overflow-y-auto custom-scrollbar flex flex-col gap-6">
        ...
        {isAdmin && (
          <div>
            ...
            {/* Staff Portal — prominent link in the Admin section */}
            <div className="mt-2 px-1">
              <Link
                href="/staff"
                className="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 group"
              >
                <div className="w-4 h-4 flex-shrink-0 relative flex items-center justify-center">
                  <ExternalLink className="w-4 h-4 text-emerald-500 group-hover:text-emerald-600 transition" />
                </div>
                Staff Portal
      ...
      <div className="p-4 pt-2">
        <Link
          ...
          className="flex items-center justify-between bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors p-3 rounded-xl group"
        >
